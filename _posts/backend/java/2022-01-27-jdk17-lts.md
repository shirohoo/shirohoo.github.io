...
특이사항으로는 JDK 11 + Spring Boot 2 조합에서 JDK 17로 업그레이드하면 호환성 에러가 많이 발생할 줄 알았는데, 의외로 하위호환성이 매우 훌륭하다고 느꼈다.

필자는 Spring Boot 2.6.3에 JDK 17을 사용했을 경우 별다른 문제없이 잘 돌아가는 것을 확인했는데, 조금 더 크고 복잡한 시스템이라면 어땟을지 또 모르겠다.
