...
키로 비교를 할 것인지, 머리카락의 길이로 비교를 할 것인지 등등이요.
...
여기서 `Student`에는 `id`라는 속성이 있기 때문에 저는 `Student`끼리 비교할 때 id를 기준으로 비교하도록 하겠습니다.
...
대부분의 컴퓨팅 시스템은 기본적으로 `오름차순(ASC)`을 기준으로 세팅이 돼있습니다. 이는 자바도 동일하고요.
...
정렬은 기본적으로 왼쪽에서 오른쪽으로 비교해나가면서 이뤄지기 때문에 5와 2를 비교할 것입니다.

따라서, 정렬 구현체는 내부적으로 `Comparator.compare(5, 2)`를 호출하고, 위 코드대로라면 이에 대한 결과는 첫번째 인수인 5에서 두번째 인수인 2를 뺀 `양의 정수` 3이 반환됩니다.

즉, `Comparator`의 명세대로라면 배열의 선행 원소인 5가 후행 원소인 3보다 크다는 의미와 동일합니다.

그리고 기본적인 정렬 방식은 오름차순이기 때문에 정렬 구현체는 5와 2의 위치를 바꾸어 더 큰 수인 5가 오른쪽으로 가도록 할 것입니다.
...
다만, `Comparator`는 비교하려는 객체 두개를 인수로 받지만, `Comparable`는 비교하려는 객체에 서브타이핑하고 다른 객체를 입력받아 자기 자신과 비교한다는 사용방법의 차이가 있을 뿐입니다.
...
마지막으로 본문과는 약간 별도의 내용이지만, 자바의 정렬은 정렬 메서드로 넘어간 배열의 사이즈에 따라 다른 정렬 알고리즘이 사용됩니다.
