...
백엔드 서버는 구축이 되어 특정한 도메인을 갖고 24시간 가동중인데, 프론트 서버가 아직 구축되지 않은 상태라 프론트 팀에서는 각자의 로컬에 프론트 서버를 띄우고 작업하고 있었기 때문에 명시할 URL이 마땅찮았습니다.
...
위 내용에 대해 설명드리자면, `사전요청(Preflight)`에는 쿠키가 없으므로 (Spring에서 사용하는 Tomcat은 `JSESSIONID`이라는 이름으로 쿠키를 내려줍니다.) CORS는 반드시 스프링 시큐리티보다 먼저 체크되고 처리되어야 한다는 것입니다.
