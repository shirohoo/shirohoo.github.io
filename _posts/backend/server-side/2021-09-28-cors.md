...
만약 CORS 정책 마저 만족하지 못한다면 SOP를 근거로 타 출처로의 리소스 요청이 금지되고 웹 브라우저 콘솔창에는 CORS 관련 에러메시지가 표시되게 됩니다.
...
물론 완벽하게 정확하진 않겠지만, `CORS`를 아주 간단하게 요약해보자면 `다른 웹 사이트의 리소스를 가져오는 것`을 의미합니다.
...
`URL`에는 정말 많은 요소가 존재하지만, 기본적으로 아래의 요소가 모두 같은 경우 같은 `출처(Origin)`이라고 부릅니다.
...
`사전요청(Preflight)`은 `HTTPHeader("Access-Control-Request-Method")`, `HTTPHeader("Access-Control-Request-Headers")`, `HTTPHeader("Origin")` 총 3가지의 `HTTP request headers`를 사용하는 `HTTPMethod("OPTIONS")` 요청입니다.

우리가 자바스크립트의 `fetch`나 `axios`등을 사용하여 브라우저에게 타 출처의 리소스를 받아오라는 명령을 내리면 브라우저는 서버에게 사전요청을 먼저 보내고, 서버는 사전요청에 대한 응답으로 현재 자신이 어떤 것들을 허용하고, 어떤 것들을 허용하지 않는지에 대한 정보를 응답 헤더에 담아서 브라우저에게 다시 보내주게 됩니다.
...
브라우저는 자신이 보낸 사전요청과 서버가 응답에 담아준 내용을 비교한 후, 이후의 요청을 보내는 것이 가능하며 안전하다고 판단되면 그제야 같은 `진입점(Endpoint)`으로 본 요청을 보내게 됩니다. 

이후 서버가 이 본 요청에 대한 응답을 하면 브라우저는 최종적으로 이 응답 데이터를 자바스크립트에게 넘겨줍니다.
...
login 엔드포인트에 대해 사전요청이 먼저 발생하고, login 엔드포인트로 본 요청이 전송됐음을 확인할 수 있었습니다.

<br />

만약 서버에서 사전요청에 대한 처리가 돼있지 않다면 사전요청 시 아래와 같은 에러 메시지가 발생하게 됩니다.
...
2,3 조건을 보니 기본적인 HTTP 헤더를 제외한 다른 헤더를 사용하면 안된다는 것은 사실상 단순요청이 발생하는 상황을 만들기가 어려울 것 같다는 생각이 듭니다.

대체로 API 서버를 개발하면 `Content-Type`은 `application/json`이나 `text/xml`을 사용하기 때문이죠.

이 시나리오로 진행할 수 있다면 하는게 좋겠지만, 현실적으로 어려워보이니 그냥 이런것도 있구나 정도로 이해하고 넘기면 될만한 내용인 것 같습니다.
...
여기서 `자격증명(Credential)` 이라는 것은, 쿠키나 인증과 관련된 헤더(인증키 등)를 의미하는데, 기본적으로 브라우저가 제공하는 `XMLHttpRequest`나 `Fetch API`를 사용하게 되면 요청에 자격증명을 담지 않습니다.
...
이 부분은 코드로 보는게 더 이해하기 쉬울 것 같습니다.
...
이렇게 보면 매우 간단하지만 이 경우 정말 골때리는 문제가 발생합니다. (제가 이번에 삽질을 하게 된 문제 중 하나가 이 문제였습니다. 😭)
...
어떤 경우에라도 CORS 헤더를 응답하는것을 허용하겠다고 했는데 오히려 와일드카드를 사용했기 때문에 안된다니? 처음에 이 부분이 정말 이해가 안되서 삽질을 했던거죠.

바로 CORS 시나리오 중 HTTP 요청에 `자격증명(Credential)`이 포함됐을 경우에 대한 내용을 이해하지 못했기 때문이었습니다.

이 시나리오에서는 두가지 규칙을 반드시 지켜야 합니다.
...
그게 뭐냐면, 백엔드 서버는 구축이 되어 24시간 가동중인데, 프론트 서버가 아직 구축되지 않은 상태라 프론트 팀에서는 각자의 로컬에 프론트 서버를 띄우고 작업하고 있었기 때문입니다.

따라서 명시할 URL이 없었습니다.

로컬에서 리액트 프로젝트를 `npm run dev`등의 명령어를 통해 띄울 경우 `localhost:3000`으로 서버가 뜨니 생각나는 URL이래봐야 `localhost`라던가 `127.0.0.1` 정도뿐인데, 해봤자 안될게 뻔했기 때문이죠. (내부적으로 `hosts` 파일을 통해 라우팅되는 도메인이므로...😫)

혹은, 프론트 개발자들의 로컬서버 공인 IP를 전달받아 모두 등록하는 방법뿐이었습니다.
...
CORS관련 문제의 원인은 알았는데, 해결 방법을 찾지 못하던 중 다행히 `스프링 시큐리티(Spring Security)`의 문서에서 이러한 상황을 해결할 수 있는 방법을 찾아냈습니다.
...
스프링 MVC 공식 문서에의 [CORS 섹션](https://docs.spring.io/spring-framework/docs/current/reference/html/web.html#mvc-cors) 에서는 CORS를 허용하려면 다음과 같은 코드를 작성하라고 하죠.
...
간단하게, `사전요청(Preflight)`에는 쿠키가 없으므로 (Spring에서 사용하는 Tomcat은 `JSESSIONID`이라는 이름으로 쿠키를 내려줍니다.) CORS는 반드시 스프링 시큐리티보다 먼저 체크되고 처리되어야 한다는 것입니다.
...
저는 스프링 시큐리티를 사용하므로 모든 인증 처리를 스프링 시큐리티에서 함께 통합관리하고 싶어 스프링 MVC를 사용한 방식이 아닌 스프링 시큐리티의 `CorsFilter`를 설정했고, 아래와 같은 코드를 추가했습니다.
...
public CorsConfigurationSource corsConfigurationSource() {
    ...
    configuration.setAllowCredentials(true); // 사용자 자격 증명(쿠키, 인증키)을 사용할 것
...
위 코드가 문제였는데, 다행히 스프링 시큐리티 팀에서 해당 문제를 회피할 수 있는 `setAllowedOriginPatterns`라는 API를 하나 제공하고 있었습니다.
...
즉, `Origin` 자체를 명시해야 하는데, 명시해야 할 `Origin`이 마땅치 않으니 이를 패턴을 통해 보다 더 유연한 방법으로 요청을 구분하고 허용하는 것입니다.

간단히 말해 위와 아래의 차이는 `서버로 오는 모든 요청에 대해 허용`하느냐, `서버로 오는 요청 중 특정한 패턴을 만족하는 요청만 허용`하느냐의 차이인 것이죠...

저는 근데 여기다가 와일카드를 사용했고, 이는 `서버로 오는 요청 중 모든 패턴의 요청을 허용`이 되므로 결과적으로 말장난 같긴 합니다 ㅎㅎㅎ 🤣
...
CORS 문제를 해결했음에도 불구하고 크롬 개발자도구에서 서버에서 응답한 쿠키를 찾을 수 없었기 때문이었습니다.
...
원칙적으로 `SOP`에 의해 `출처(Origin)`가 다른 서버에서 리소스를 얻어올 수 없는데, 이를 `CORS`라는 일종의 위법성 조각 사유를 만족해 임의의 리소스를 얻어오게 되면 자연스럽게 `XSS`, `CSRF` 등의 공격에 취약해지게 됩니다.
...
`Microsoft`가 `2002`년에 `MS Internet Explorer 6.0 SP1`에 적용한 이후로 현재는 대부분의 웹 브라우저가 지원하는 보안 기법이라고 합니다.
...
하지만 `HttpOnly`는 `XSS`와 `CSRF` 공격에 대해 최소한의 방어는 보장하지만, 완벽한 방어는 보장하지 않기 때문에, 보안 공격을 고려한다면 `XSS Filter`, `CSRF Filter`등의 추가적인 보안 시스템을 구축해야 합니다.

<br />

정리하자면 CORS 문제를 해결했고, 인증과정을 거쳐 서버에서는 정상적으로 쿠키를 응답하였으나 해당 쿠키는 `HttpOnly`가 적용된 보안 쿠키였기 때문에, 클라이언트 환경에서 해당 쿠키가 노출되지 않는 상태였습니다.
