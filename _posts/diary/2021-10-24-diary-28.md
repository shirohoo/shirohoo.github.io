...
라는 생각이 많이 들었고, 여기에서 동기부여가 잘 되지 않았던 것 같다.
...
**"아! 이렇게 공부해봐야 이건 시간낭비일 뿐이고 아무런 의미가 없다. 다시 처음으로 돌아가자. 하나를 하더라도 제대로 해보자."**

<br />

관련 아티클을 최대한 찾아보면서 어떻게 진행해야 하는지를 먼저 알아냈다.
...
배타적 논리합(XOR)은 두 입력이 다를 경우에만 참을 반환한다. 이 점에 착안해 진리표를 먼저 그린다.
...
이 때 결과가 1이 나오는 경우만 따져서 식을 도출한다.
...
**"이걸 배워서 어디다 써먹어야 될까?"** 같은 생각을 떠나서 그냥 참 재미있다고 느꼈다. CS지식을 공부하면서 이런 감정을 느껴본건 처음이었던 것 같다.

이것을 해내기 위해 수시간 동안 `디지털논리회로` 과목의 기초를 보며 여러가지를 학습해야 했는데,

`명제`, `카르노맵`, `분배 법칙`, `결합 법칙`, `흡수 법칙`, `교환 법칙`, `드모르간의 법칙` 등과, 컴퓨터가 사칙연산을 어떻게 덧셈으로만 다 해낼 수 있는지.

`반가산기`, `전가산기` 등이 왜 중요한지 등등 정말 많은 개념을 학습해야만 했다.

아무튼 이런 개념들을 학습하고 즉시 코드에 적용 해 결과물을 받아볼 수 있다는 것은, 내가 현재 학습하고 있는 지식들이 죽은 지식이 아닌 살아있는 지식임을 느끼게 해주었고, 이게 흥미와 재미로 다가왔다.

<br />

2진수에 대해 학습하다 문득 승재님과 재미있는 것을 하나 했다.

<u>우리가 학습한 내용을 어떻게 응용해야 홀수 짝수 연산을 더 효율적으로 할 수 있을지</u> 였다.
...
이 발상에 착안 해 인수가 짝수라면 true를 반환하고 홀수라면 false를 반환하는 코드를 자바로 구현하니 다음과 같은 코드가 나왔다.
...
그리고 이 코드의 성능이 궁금해져 JMH로 이 코드의 성능 측정을 해봤다.
...
이 스터디를 진행하다 보니 학습하는 CS 지식들이 개발에 즉각적인 도움을 주지는 않는 것 같다.

다만, 이러한 것들을 위해 계속 해야만 하는 논리적인 사고가 내 사고를 점점 효율적으로 만들어준다는 것은 알 수 있었다.

또한, 이런 논리적이고 효율적인 사고들이 개발에 대한 내 생각을 조금씩 바꿔가는 게 느껴진다.
