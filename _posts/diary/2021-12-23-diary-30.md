...
많은 결제 벤더사를 한 시스템에 연동해야하는 일이었다.

우리 회사 앱에서 내가 만들어야 할 서비스에 결제 요청을 보내면 이를 처리하고 응답을 반환해야 했다.
...
- <u>미래에 외부 벤더사가 수십~수백개가 될수도 있다</u>
...
- 각 벤더사에 요청을 보낼때 수수료가 발생하며 이는 각 벤더사마다 다르다
