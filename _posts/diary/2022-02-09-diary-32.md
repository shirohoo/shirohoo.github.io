...
나는 최대한 소프트웨어 개발 3대 원칙과 객체지향 5대 원칙(SOLID)을 고민하면서 훈련 할 것이다.
...
우선 이 게임에서 가장 중요한 객체인 숫자를 구현해야 겠다.

클래스 이름은 `Numbers`라고 명명하였고, 객체의 유연함을 위해 외부에서 생성 전략을 주입받아 생성되도록 구성할 것이다.
...
1. 우선 생성된 세 숫자의 중복을 제거하고도 숫자의 세개가 3개여야 할 것이다.
2. 1~9사이의 중복되지 않는 숫자 3개가 나오므로 최소로 나올 수 있는 수는 `123`이며, 최대로 나올 수 있는 수는 `789`이다. 따라서 세 숫자의 합의 범위가 `6 <= 수 <= 24` 면 된다. 
3. 1,2번 테스트 케이스로도 충분히 검증은 될 것 같지만, 아무래도 랜덤수이다 보니 조금 더 신뢰성을 확보하기 위해 테스트를 100번 반복해야 겠다.
...
바로 구현에 들어간다. 위 테스트만 만족하면 `RandomNumbersGenerativeStrategy`는 유효하다고 볼 수 있을 것 같다.

구현은 쉽게 되었다. 랜덤수를 생성할 때 자바 17에 포함되어 릴리즈된 `의사난수 생성기`를 사용해봤다.
...
데이터 중심적인 사고를 버리고 행동과 책임 중심적인 사고를 길러야 한다 !
...
record 클래스로 변경하고 다시 테스트를 실행해보자.
...
사용자 입력이 null, empty, 3자리 초과, 중복되는 수, 0이 포함된 경우 예외가 발생되도록 할 것이다.
...
심판은 하나의 행동만 하며, 이 행동은 두개의 숫자를 비교하여 `판정(decision)`하는 것이다.

볼, 스트라이크가 나올 수 있는 경우의 수를 최대한 추가하여 테스트 케이스를 작성한다.

`심판(Referee)`은 판정 후 `판정 결과(DecisionResult)`를 반환 할 것이다.
...
이를 유저가 `숫자 야구 게임(BaseBall)`을 한번 시도했음으로 표현하여 `action()`이라고 정의해봐야 겠다. 

경험상 유스케이스는 이미 테스트 코드로 검증된 도메인 클래스들을 갖다 사용하므로 바로 구현에 들어가도 무방하다.

이미 테스트 된 것은 다시 테스트하지 말라는 말이 있기 때문이다. (DRY 원칙)  

유스케이스 클래스가 생성될 때 심판과 컴퓨터 랜덤수를 포함하고, 사용자 입력을 받아 계속 결과를 반환하도록 작성해보자.
...
이러면 유저가 입력하는 데이터를 외부에 직접적으로 노출하게 되어 외부에서는 항상 `String`으로만 입력할 수 밖에 없게 된다.

이를 `UserInput`으로 추상화하고 `Numbers`도 변경해주어야겠다.

그리고, `Numbers.nonOverlapping3digits()`에 있던 유효성 검사 코드를 모두 `UserInput`으로 옮겨와야겠다.
...
이제 마지막으로 콘솔 입력과 출력을 구현하고, 콘솔 애플리케이션을 완성하자.

이부분은 요구사항에 따라 테스트를 생략하고 바로 구현하도록 해야겠다.
...
스트라이크를 판별하는 부분이 별로 마음에 안든다.

`DecisionResult`에 스트라이크 카운트가 3이면 스트라이크 아웃인지 판별하도록 위임해야겠다.

항상 그랬듯이 우선 테스트 케이스를 하나 작성한다.
