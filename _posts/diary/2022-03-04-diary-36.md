...
자바 고급 서적들(JVM 레벨의...)을 보다보면 `커널 스레드`니 `유저 스레드`니 `그린 스레드`니 혹은 그냥 '스레드'니 하는 용어들이 미친듯이 나오는데, 기본기가 없으니 이게 대체 무슨말들을 하는건지 알아들을수가 있어야지...
...
아무튼, 이러한 이유로 `운영체제` 과목을 공부하게 됐는데, 까먹으면 내가 나중에 다시 볼 거기도 하고, 나 같은 분들(노베이스 비전공자)께 도움이 되길 바라기도 하며 나름대로 정리한 내용들을 기록해본다.
...
- `병렬성(Parallel)`
  - CPU가 동 시간대에 여러개의 작업을 처리하고 있는 것. 즉, CPU1이 프로세스1을 처리하고, CPU2가 프로세스2를 처리하면 동 시간대에 정확히 두개의 작업을 처리하고 있는 것이다. 이를 병렬성이라고 부른다
...
HT/SMT는 부르는 것은 실제로 물리적인 코어는 하나만 있지만, 운영체제에게는 코어가 두개인 것처럼 속이는 것이다.
...
이것들이 모두 같은걸 의미하는 용어라는걸 확신할 수 없는 상황(노베이스 비전공 -ㅅ-)에서 온갖 글들을 보고있으면 머리만 더 아파지는 것이다. (예를 들자면... 잉? 이건 또 뭐야?)

<u>이제와서 나는 대부분의 상황에서 커널 스레드를 위의 용어들과 동일하다고 생각하며 보고 있으며, 실제로 그렇게 봐도 무방했다.</u>
...
운영체제도 일단 프로그램이기 때문에, 내부적으로 최소 한개 이상의 스레드를 가진다.

운영체제가 프로그램으로서 갖는 스레드를 바로 `커널 스레드(혹은 위에 나열한 용어들...)`라고 부르며, 운영체제 코드에 작성 된 `CPU 스케쥴링`같은 코드들에 직접적으로 영향을 받아 하드웨어 스레드를 점유하는 것이 바로 이 커널 스레드이다.

그러니까, 결과적으로 이 `커널 스레드`들이 위에서 말한 `하드웨어 스레드`와 매핑되어 컴퓨터를 동작시키게 된다. 

<u>즉, 4코어 8스레드짜리 컴퓨터에서는 동시간대에 정확히 8개의 커널 스레드가 동작할 수 있는 것이다.</u>

따라서 멀티코어 파워를 온전히 활용하는 병렬 프로그래밍에 있어 매우 중요한 개념이라고 볼 수 있겠다.
...
프로그래밍 언어로 프로그램을 개발하면, 내부적으로 최소 한개 이상의 스레드를 갖는데, 이렇게 추가적인 스레드를 만들고 제어하는 것들을 대부분의 프로그래밍 언어에서 지원하고 있다.
...
모던 자바의 스레드는 약간 특이한데, `Thread.start()`가 호출되면 `start0()` 이라는 `JNI(Java Native Interface)`를 통해 운영체제의 `시스템 콜(System Call)`을 호출해 커널 스레드를 직접적으로 생성하여 유저 스레드(여기선 자바 스레드)와 매핑한다.

즉, 모던 자바의 모든 유저 스레드는 커널을 직접 만들어 다이렉트로 연결되므로, 하나도 남김없이 커널의 CPU 스케쥴링에 직접적으로 영향받는다.
