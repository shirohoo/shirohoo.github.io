...
- 프로그램: 코드로 작성된 실행될 수 있는 애플리케이션. 하지만 메모리(RAM)를 할당받지 못하고 하드디스크에 저장돼있는 것
- 프로세스: 프로그램이 운영체제로부터 메모리을 할당받아 실행된 것. 즉, 실행 된 프로그램
- 스레드: 프로세스의 실행 단위. 하나의 프로세스는 최소한 하나 이상의 스레드를 포함한다
- 멀티프로그래밍 or 멀티프로세스: 메모리에 여러개의 프로세스가 올라가는 것
- 멀티태스킹: CPU가 여러개의 프로세스를 동시에 실행하는 것. CPU가 프로세스1을 조금 작업하고 정지한 후 프로세스2를 작업하러 가는 것
- 시분할 시스템(Time Sharing): 멀티태스킹을 위해 각 프로세스가 CPU를 점유할 수 있는 시간을 강제해둔 시스템
- 멀티프로세싱: 프로그램이 여러개의 CPU(=프로세서)를 활용하는 것. 병렬 프로그래밍이라고도 불린다
- 멀티스레딩: 하나의 프로세스가 여러개의 스레드를 갖는 것. 멀티스레딩을 활용하면 동시성을 보장할 수 있다
- 동시성: CPU가 동 시간대에는 단 하나의 작업을 처리하지만, 여러개의 작업을 조금씩 번갈아가며 처리하면 이 작업이 인간 입장에선 너무 빠르기 때문에 여러개의 작업이 `동시에` 처리되는 것 처럼 느낀다
- 병렬성: CPU가 동 시간대에 여러개의 작업을 처리하고 있는 것. 즉, CPU1이 프로세스1을 처리하고, CPU2가 프로세스2를 처리하면 동 시간대에 정확히 두개의 작업을 처리하고 
...
즉, 하드웨어가 발전함에 따라 CPU가 일하는 속도가 너무 빨라졌기 때문에, CPU가 할 일이 없어 손가락을 빨며 대기하는 상황이 생긴다는 것이다.
...
즉, CPU의 성능을 온전하게 활용하지 못하고 낭비하게 되는 것이다.
...
따라서 병렬 프로그래밍에 있어 매우 중요한 개념이라고 볼 수 있겠다.
...
그러니까 실제적으로 유저 스레드가 작업을 하려면 커널 스레드와 반드시 매핑이 되어야만 하며(CPU가 커널 스레드를 바라보므로), 커널 스레드와 매핑되지 않은 유저 스레드는 아무런 작업도 할 수 없다.
