...
`프록시`는 이 인디렉션과 궤를 같이하며 프록시 패턴은 SOLID중 `개방폐쇄원칙(OCP)`과 `의존역전원칙(DIP)`를 충실히 따른다.
...
두 개체는 사이에 프록시라는 가짜 개체가 있는지 전혀 모르며 서로를 진짜라고 신뢰하고 통신을 진행하게 된다.
...
데이터베이스의 테이블과 자바의 클래스를 매핑하여 데이터베이스를 객체지향적으로 사용하기 위한 방법이다.

이 방법을 사용하게 되면 데이터베이스의 외래키를 자바 코드로 구현할 때 자바의 객체 그래프 형식으로 변환된다.
