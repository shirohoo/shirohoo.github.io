...
정보처리기사 데이터베이스 과목에서 `정규화`는 굉장히 비중있게 나오는 내용이지만, 실제 현업에서는 일반적으로 정규화를 잘 하지 않는다.
...
그리고 부가적인 장점으로 테이블의 용량이 최적화된다는 점이 있다.

&nbsp;  

하지만 정규화를 하게되면 생기는 단점이 너무 크다. 

테이블이 잘게 쪼개지는 만큼 `조인(join)`을 해야하며, 많은 조인은 데이터베이스에 부하를 일으킨다.

또한, 조인으로 인해 트랜잭션의 `로킹 단위`가 커짐으로 인해 인해 데이터베이스의 공유도가 감소하여 `동시성 성능`이 떨어지게 된다.

이는 곧 `질의(query)`에 대한 응답속도가 느려짐을 뜻한다.

&nbsp;  

서비스를 운영하는 회사 입장에서 고객이 느끼는 속도의 저하는 굉장히 커다란 이슈이기도 하고,

<u>데이터베이스에 많은 부하가 몰린다는 것은 전체 서비스의 다운을 초래할수도 있기 때문에 결과적으로 정규화를 되도록 하지 않는 쪽으로 가는 것 같다.</u>

&nbsp;  

**"값 타입 이야기 하다 말고 뜬금없이 왜 정규화 타령이냐?"** 라는 생각이 들수도 있다.

&nbsp;  

위의 문제로 인해 <u>테이블 정규화를 하지 않음으로서 실무에서 보는 테이블들은 컬럼이 수십개 이상인 경우가 굉장히 많다.</u>
...
`JPA`를 `ORM`답게 사용할 경우 `서비스 레이어(Service Layer)`에는 코드가 별로 없고, 대부분의 비즈니스 로직이 엔티티 클래스안에 들어있기 때문에 서비스 클래스는 일종의 `퍼사드 패턴(Facade Pattern)`처럼 변하게 된다.

&nbsp;  

즉, 중요한 비즈니스 로직들을 각각의 엔티티 클래스가 갖고있는 상태에서 서비스 클래스는 엔티티들을 한데 모아 하나의 `트랜잭션(Transaction)`으로 묶어줌과 동시에 `흐름제어`를 하는 역할을 주로 맡게된다.
...
이렇게 사용함으로써 얻는 한가지 더 큰 이점은, <u>프로젝트 전체에서 재사용 할 수 있다는 점이다.</u>
...
`customer1` 의 값만 안전하게 변경된 것을 확인 할 수 있다.
