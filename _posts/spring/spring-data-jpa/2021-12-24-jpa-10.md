...
이는 `Hibernate`가 `프록시`를 채워주어야 할지 `NULL`을 넣어야 할지 알지 못해 쿼리를 때려보는(Eager 동작으로 인한 N+1) 불상사가 일어나지 않을 수 있는 기반을 마련해준것이다.
...
그리고 복잡한 실무환경에서 JPA를 사용하다 보면 결국 코드만 봐서는 정확히 어떤 쿼리가 발생할 것인지 완벽하게 알기가 어렵다.
